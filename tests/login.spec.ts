...
import { adminUser, invalidUser, nonAdminUser } from '../test-data/users';
...
testNoAuth.describe('Login', () => {
  ...
  testNoAuth('shows error message for non-existing credentials', async ({ loginPage }) => {
    await loginPage.login(invalidUser.username, invalidUser.password);
    await expect(loginPage.badCredentialsMessage).toBeVisible();
    await expectLoginFormVisible(loginPage);
  });

  testNoAuth('shows error message for valid email and wrong password', async ({ loginPage }) => {
  ...
  testNoAuth(
    'shows error message for submitions with empty email and password',
    async ({ loginPage }) => {
  ...
  testNoAuth('shows error message for submitions with empty email', async ({ loginPage }) => {
    await loginPage.login('', adminUser.password);
    await expect(loginPage.badCredentialsMessage).toBeVisible();
    await expectLoginFormVisible(loginPage);
  });

  testNoAuth('shows error message for submitions with empty password', async ({ loginPage }) => {
